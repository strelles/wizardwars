...
public class UISpriteAnimation : MonoBehaviour
{
	...
	protected virtual void Update ()
	{
		if (mActive && mSpriteNames.Count > 1 && Application.isPlaying && mFPS > 0)
		{
			...
			if (rate < mDelta)
			{
				...
				if (mActive && !PlayInReworse)
				{
					mSprite.spriteName = mSpriteNames[mIndex];
					if (mSnap) mSprite.MakePixelPerfect();
				}
                else if(mActive && PlayInReworse)
                {
                    mSprite.spriteName = mSpriteNames[mSpriteNames.Count - mIndex - 1];
                    if (mSnap) mSprite.MakePixelPerfect();
                }
			}
		}
    ...
    public void Play () { mActive = true; PlayInReworse = false; }

    public void Play (bool playInReworse) {
        mActive = true;
        PlayInReworse = playInReworse;
    }
	/// <summary>
	/// Pause the animation.
	/// </summary>
